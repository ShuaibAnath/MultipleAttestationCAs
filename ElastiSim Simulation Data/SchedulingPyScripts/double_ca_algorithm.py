...
# availability_timer_stopped = False
# availability_start_time = 0.0
# availability_total_time = 0.0


jobs_assigned_cas = {}
queue_data_double_df = pd.DataFrame(columns=['Time', 'queue_length','load','Running Jobs','Invocation Type', 'Allocated CA Nodes','Job ID'])
# queue_data_double_df = pd.DataFrame(columns=['Submit Time', 'queue_length','load','Job ID'])
# load_data_double_df = pd.DataFrame(columns=['End Time', 'queue_length','load','Job ID'])
# unavailability_data_double_df = pd.DataFrame(columns=['Start Time','Current Time','Total Unavailabililty Time','queue_length','Allocated CA Nodes','Invocation Type'])
# availability_data_double_df = pd.DataFrame(columns=['Start Time','Current Time','Total Availabililty Time', 'queue_length','Allocated CA Nodes'])
...
def schedule(jobs: list[Job], nodes: list[Node], system: dict[str, Any]) -> None: 
    ...
    global queue_data_double_df
    # global load_data_double_df
    ...
    # global availability_timer_stopped
    # global availability_data_double_df
    # global availability_total_time
    # global availability_start_time
    # global availability_stop_time
    ...
    allocated_nodes = [node.identifier for node in nodes if node.state == NodeState.ALLOCATED and node.identifier < 2]
    ...
    queue_data_double_df.loc[len(queue_data_double_df)] = [time, len(pending_jobs), len(pending_jobs) + len(running_jobs),len(running_jobs),
    ...
    # if len(pending_jobs) >= QUEUE_LENGTH and nodes[0].state == NodeState.ALLOCATED and nodes[1].state == NodeState.ALLOCATED:
    ...
    # elif  len(pending_jobs) < QUEUE_LENGTH or nodes[0].state == NodeState.FREE or nodes[1].state == NodeState.FREE:
    #     if unavailability_timer_started:
    #         unavailability_total_time = unavailability_total_time + time - unavailability_start_time
    #     unavailability_timer_started = False
    #     unavailability_data_double_df.loc[len(unavailability_data_double_df)] = [unavailability_start_time, time, unavailability_total_time, len(pending_jobs), allocated_nodes,invocation_type]
    ...
    #     queue_data_double_df.loc[len(queue_data_double_df)] = [time, len(pending_jobs), len(pending_jobs) + len(running_jobs)]

    # if invocation_type == InvocationType.INVOKE_JOB_SUBMIT:
    #     job_on_invocation = system['job']
    #     queue_data_double_df.loc[len(queue_data_double_df)] = [time, len(pending_jobs), len(pending_jobs) + len(running_jobs),f'job{job_on_invocation.identifier}']
    
    # if  invocation_type == InvocationType.INVOKE_JOB_COMPLETED:
    #     job_on_invocation = system['job']
    #     load_data_double_df.loc[len(load_data_double_df)] = [time, len(pending_jobs), len(pending_jobs) + len(running_jobs), f'job{job_on_invocation.identifier}']
...
if __name__ == '__main__':
    ...
    queue_data_double_df.to_csv(f'queue_data_double_arrival_{arrival_rate}.csv', index=False)
    # load_data_double_df.to_csv('load_data_double.csv', index=False)
    # unavailability_data_double_df.to_csv('unavailability_data_double.csv', index=False)
    # availability_data_double_df.to_csv('availability_data_double.csv', index=False)